...
class paloACL {
    ...
    function getUsers($id_user = NULL)
    {
        return $this->_getRowsById(
            'acl_user',
            array('id', 'name', 'description', 'extension', 'nuevo_campo'),
            $id_user);
    }
    ...
    function getUsersPaging($limit, $offset)
    {
        ...
        $this->errMsg = "";
        $sPeticionSQL = "SELECT id, name, description,extension,nuevo_campo FROM acl_user limit ? offset ?";
     ...
     */
    function createUser($username, $description, $md5_password, $nuevo_campo, $extension = NULL)
    {
        $bExito = FALSE;
        if ($username == "") {
...
//            $this->errMsg = "Clave de acceso no es un hash MD5 valido";
            ...
            if ($id_user !== FALSE) {
                ...
                $sPeticionSQL = "INSERT into acl_user (name,description,md5_password,extension,nuevo_campo) VALUES (?,?,?,?,?)";
                $arrParam = array($username,$description,$md5_password,$extension,$nuevo_campo);
     ...
     */
    function updateUser($id_user, $username, $description, $nuevo_campo, $extension = NULL)
    {
        $bExito = FALSE;
        if ($username == "") {
            $this->errMsg = "UserName can't be empty";
            ...
            if (!is_array($tuplaUser)) {
                ...
                if ($bContinuar) {
                    ...
                    $sPeticionSQL = "UPDATE acl_user SET name = ?, description = ?, nuevo_campo = ? WHERE id = ?";
                    $arrParam = array($username,$description,$nuevo_campo,$id_user);
                    if (!$this->_DB->genQuery($sPeticionSQL,$arrParam)) {
                        $this->errMsg = $this->_DB->errMsg;
                    } else {
