    ...
    private function _buildGridChildren($pACL, $customurlprefix, $children, &$grid, $nlevel, $idparent, $admin)
    {
        ...
        foreach (array_keys($children) as $key) {
            ...
            $tupla = array(
                ...
                '&nbsp;',
                '&nbsp;',
                //htmlspecialchars($key, ENT_COMPAT, 'UTF-8'), // Columna Recurso
            ...
            if ($children[$key]['HasChild']) {
                ...
                if (is_array($privlist) && count($privlist) > 0) {
                    //Agrega los Privilegios personalizados => viewany, create, editany, deleteany
...
function reportGroupPermission($smarty, $module_name, $local_templates_dir)
{
    ...
    //$oGrid->setColumns(array('', '', '', _tr('Resource'), _tr('Description'), _tr('Available'), _tr('Enabled'), _tr('Custom Privileges')));
    $oGrid->setColumns(array('', '', '', '',/* Columna Recurso*/ _tr('Description'), _tr('Available'), _tr('Enabled'), ''));
