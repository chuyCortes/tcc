...
function listUsers($pACL, $smarty, $module_name, $local_templates_dir, $plugins)
{
    ...
    $arrColumns = array('', _tr("Login"), _tr("Real Name"), _tr("Group"), _tr("Campo Extra"));
    ...
    foreach ($arrUsers as $user) {
        ...
        $arrTmp = array(
            ...
            htmlentities($user[4], ENT_COMPAT, 'UTF-8'),
        );
...
function editUser($pACL, $smarty, $module_name, $local_templates_dir, $plugins){
    ...
    return createEditUser($pACL, $smarty, $module_name, $local_templates_dir, $id_user, $privileged, $plugins, $algo);
}
    ...
    $privileged, $plugins, $algo)
{
        ...
        if (!$oForm->validateForm($_POST)) {
            ...
            if (is_null($id_user)) {
                $r = $pACL->createUser($_POST['name'], $_POST['description'],
                    md5($_POST['password1']),$_POST['nuevo_campo']);
                ...
                $r = $pACL->updateUser($id_user, $userinfo['name'], $_POST['description'], $_POST['nuevo_campo']);
    ...
    if(is_null($id_user)){
        $smarty->assign(array('LBL_CORE_FIELDS'   =>  _tr('New User')));
    } else {
        $smarty->assign(array('LBL_CORE_FIELDS'   =>  _tr('Edit User')));
    }
...
function createFormFields($arrGrupos)
{
    return array(
        "description" => array(
            ...
            "REQUIRED"               => "no",
            "INPUT_TYPE"             => "TEXT",
            "INPUT_EXTRA_PARAM"      => array("class" => "form-control", "autofocus" => ""),
        ...
        "password2"   => array(
         ...
         "group"       => array(
            ...
            "VALIDATION_EXTRA_PARAM" => ""),
         "nuevo_campo"       => array(
            "LABEL"                  => _tr("Nuevo Campo"),
            "REQUIRED"               => "yes",
            "INPUT_TYPE"             => "TEXT",
            "INPUT_EXTRA_PARAM"      => array("class" => "form-control"),
            "VALIDATION_TYPE"        => "text",
            "VALIDATION_EXTRA_PARAM" => ""),
    );
}
